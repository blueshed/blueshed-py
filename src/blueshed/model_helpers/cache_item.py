...
@author: peterb
'''
from sqlalchemy import Column, String, PickleType, Integer, BigInteger, event
...
class CacheItem(object):
    ...
    def init_cache_item(cls, session, db_name):
        ...
        rs = list(session.execute('SELECT * from innodb_memcache.containers WHERE name=:name',params))
        if len(rs) == 0:
            session.execute('INSERT INTO innodb_memcache.containers VALUES (:name,:db,:table,"c1","c2","c3","c4","c5","PRIMARY")',params)
        ...
        def _cache_before_flush(session, context, instances):
            for item in session.dirty:
                if isinstance(item, clazz):
                    ...
                    cls.update_cache(session, item.id, value, session.info.get("accl_key"))
                    if signal:
